...
const DESCRIPTION = [
  ...
  'Binary files are skipped automatically (detected by a NUL byte in the first bytes), as are files above the workspace file-size limit, symlinks, and the same heavy directories workspace.tree excludes (node_modules, vendor, dist, build, .cache, .git/objects). Paths matching the workspace secret denylist are never scanned.',
